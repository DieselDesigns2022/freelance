...
Status: the Codex implementation PR was merged into the Phase 2.2 branch, the branch was pulled to the VPS, and the production migration was applied after file and database backups. MariaDB 10.11 verified `products.intake_type` and the signed `product_live_examples.product_id` foreign key. The verified premade Shopify Revamp product, ID 1, was classified as `shopify_revamp_standard`. Public smoke checks, admin authentication redirect, intake selection, and the reorganized admin product layout passed live testing. Custom Shopify Theme runtime testing verified product creation, multiple live-example creation, ordered public display, accurate service and intake labels, and placement under Shopify Make-Overs rather than Website Builds. The reusable Questionnaire Builder and Custom Shopify Theme questionnaire are planned for Phase 2.3; Custom Website Build intake is planned for Phase 2.4.

### Phase 2.3 questionnaires
Phase 2.3 now includes the reusable compact field-card builder, inline editing, positional adds, drag/fallback ordering, selected-question imports, full-questionnaire draft copies, preview, collision-safe keys, protected single/multiple uploads, and immutable order snapshots. It also includes the `addon` field for server-authoritative integer-cent manual-invoice upgrades and the Admin Order **Manual Invoice Add-Ons** summary. Upload labels changed only in administrator displays; stored `file` and `multiple_files` values and behavior remain intact.

Implementation-level PHP lint, diff checks, focused calculator/conversion/snapshot/import tests, and static rendering review passed. The add-on migration and all authenticated/public browser, database, upload, order, contract, mobile, and accessibility checks are still pending; no live add-on verification is claimed. See [`docs/PHASE_2_3_QUESTIONNAIRES.md`](PHASE_2_3_QUESTIONNAIRES.md) and [`docs/TESTING.md`](TESTING.md).
