...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'settings') {
        ...
        $sourceId=(int)($_POST['source_field_id']??0); $operator=(string)($_POST['operator']??''); $actionType=(string)($_POST['rule_action_type']??''); $targetId=(int)($_POST['target_field_id']??0); $comparison=trim((string)($_POST['comparison_value']??'')); $feeName=trim((string)($_POST['fee_name']??'')); $feeCents=questionnaire_dollars_to_cents((string)($_POST['fee_amount']??''));
        $candidate=['source_field_id'=>$sourceId,'operator'=>$operator,'comparison_value'=>questionnaire_operator_needs_value($operator)?$comparison:null,'actions'=>[['action_type'=>$actionType,'target_field_id'=>$targetId?:null,'fee_name'=>$feeName,'fee_cents'=>$feeCents]]];
        ...
        $targetStmt=db()->prepare('SELECT * FROM questionnaire_fields WHERE id=? AND questionnaire_template_id=?');$targetStmt->execute([$targetId,$id]);$target=$targetStmt->fetch();if($target)$candidate['actions'][0]['target_field_key']=$target['field_key'];
        ...
        if($ruleErrors)$errors=array_merge($errors,$ruleErrors);else{$pdo=db();$pdo->beginTransaction();try{$pdo->prepare('INSERT INTO questionnaire_field_rules(questionnaire_template_id,source_field_id,operator,comparison_value_json,stable_key,created_at,updated_at) VALUES(?,?,?,?,?,NOW(),NOW())')->execute([$id,$sourceId,$operator,json_encode($candidate['comparison_value'],JSON_THROW_ON_ERROR),'rule_'.bin2hex(random_bytes(8))]);$ruleId=(int)$pdo->lastInsertId();$pdo->prepare('INSERT INTO questionnaire_rule_actions(rule_id,action_type,target_field_id,fee_name,fee_cents,sort_order,created_at,updated_at) VALUES(?,?,?,?,?,10,NOW(),NOW())')->execute([$ruleId,$actionType,$actionType==='fee'?null:$targetId,$actionType==='fee'?$feeName:null,$actionType==='fee'?$feeCents:null]);$pdo->commit();flash('success','Conditional rule added.');redirect('questionnaires-edit.php?id='.$id);}catch(Throwable $exception){if($pdo->inTransaction())$pdo->rollBack();$errors[]='The conditional rule could not be saved.';}}
        ...
        <div data-editor-controls <?= $type === '' ? 'hidden' : '' ?>>
            ...
            <div data-control="addon-settings">
                ...
                <div class="product-form-grid">
                    ...
                    <label data-addon-price><span data-addon-price-label>Price</span> *<input type="text" inputmode="decimal" name="price" value="<?= e($priceValue) ?>" placeholder="0.00"></label>
                    ...
                    <label data-addon-maximum><span data-addon-maximum-label>Maximum quantity customer can enter</span><input type="number" min="0" step="1" name="max_quantity" value="<?= e((string) ($validation['max_quantity'] ?? '0')) ?>"></label>
                </div>
                <p class="helper" data-addon-helper>Used only to calculate the amount you will manually invoice.</p>
            </div>
...
<div class="questionnaire-builder">
    ...
    <section class="admin-card questionnaire-conditional-logic">
        ...
        <form method="post" class="admin-form questionnaire-rule-editor" data-rule-editor>
            ...
            <label>Action<select name="rule_action_type" required data-rule-action><?php foreach(QUESTIONNAIRE_RULE_ACTIONS as $choice):?><option value="<?= e($choice) ?>"><?= e(match($choice){'show'=>'Show target field','hide'=>'Hide target field','required'=>'Make target required','optional'=>'Make target optional','fee'=>'Add a flat conditional fee'}) ?></option><?php endforeach;?></select></label>
            <label data-rule-target>Target question<select name="target_field_id"><option value="">Choose a target</option><?php foreach($fields as $targetField):if(empty($targetField['is_active']))continue;?><option value="<?= (int)$targetField['id'] ?>" data-structural="<?= in_array($targetField['field_type'],QUESTIONNAIRE_STRUCTURAL_TYPES,true)?'1':'0' ?>"><?= e($targetField['label']) ?></option><?php endforeach;?></select></label>
            <div data-rule-fee hidden><label>Conditional fee name<input name="fee_name" maxlength="180"></label><label>Fee amount in dollars<input name="fee_amount" inputmode="decimal" placeholder="40.00"></label></div>
            <button class="btn btn-accent">Add another rule</button>
        </form>
    </section>
...
(() => {
 ...
 const ruleEditor=document.querySelector('[data-rule-editor]'); if(ruleEditor){const source=ruleEditor.querySelector('[data-rule-source]'),operator=ruleEditor.querySelector('[data-rule-operator]'),comparison=ruleEditor.querySelector('[data-rule-comparison]'),options=ruleEditor.querySelector('[data-rule-options]'),action=ruleEditor.querySelector('[data-rule-action]'),target=ruleEditor.querySelector('[data-rule-target]'),fee=ruleEditor.querySelector('[data-rule-fee]');const operatorMap={short_text:['equals','not_equals','is_answered','is_blank'],long_text:['equals','not_equals','is_answered','is_blank'],email:['equals','not_equals','is_answered','is_blank'],phone:['equals','not_equals','is_answered','is_blank'],url:['equals','not_equals','is_answered','is_blank'],yes_no:['equals','not_equals','is_answered','is_blank'],dropdown:['equals','not_equals','is_answered','is_blank'],radio:['equals','not_equals','is_answered','is_blank'],checkboxes:['contains','not_contains','is_answered','is_blank'],number:['equals','not_equals','greater_than','greater_or_equal','less_than','less_or_equal','is_answered','is_blank'],date:['equals','before','on_or_before','after','on_or_after','is_answered','is_blank'],file:['has_file','has_no_file'],multiple_files:['has_file','has_no_file'],multiple_inputs:['is_answered','is_blank'],addon:['is_selected','is_not_selected','quantity_equals','quantity_greater_than','quantity_less_than']};const labels=value=>value.replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase());const updateSource=()=>{const selected=source.selectedOptions[0],values=operatorMap[selected?.dataset.type]||[];operator.innerHTML='<option value="">Choose a condition</option>'+values.map(value=>`<option value="${value}">${labels(value)}</option>`).join('');options.replaceChildren(...JSON.parse(selected?.dataset.options||'[]').map(value=>{const option=document.createElement('option');option.value=value;return option}));[...ruleEditor.querySelector('[name=target_field_id]').options].forEach(option=>option.disabled=option.value===source.value)};const updateOperator=()=>comparison.hidden=['is_answered','is_blank','has_file','has_no_file','is_selected','is_not_selected'].includes(operator.value);const updateAction=()=>{const priced=action.value==='fee';fee.hidden=!priced;target.hidden=priced;ruleEditor.querySelector('[name=target_field_id]').required=!priced;ruleEditor.querySelector('[name=fee_name]').required=priced;ruleEditor.querySelector('[name=fee_amount]').required=priced;[...ruleEditor.querySelector('[name=target_field_id]').options].forEach(option=>option.hidden=['required','optional'].includes(action.value)&&option.dataset.structural==='1')};source.addEventListener('change',updateSource);operator.addEventListener('change',updateOperator);action.addEventListener('change',updateAction);updateSource();updateOperator();updateAction();}
})();
</script>
